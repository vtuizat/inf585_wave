...
void scene_structure::initialize()
{
	...
	for (int i = 0; i < N * N * Lfactor; i++){
		floor.position.at(i)=vec3(Wfactor * floor.position.at(i).x, Lfactor * floor.position.at(i).y,  atan_floor(floor.position.at(i).y));//- 3.2);
	}
...
float atan_floor(float x){
	return 3*(std::atan(0.5*(x-10))-1.5);
}
